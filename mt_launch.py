...
from arm.custom_rlbench_env import CustomRLBenchEnv, MultiRLBenchEnv
import numpy as np
...
def _gen_short_names(cfg: DictConfig):
    names = []
...
def run_seed(cfg: DictConfig, env, cams, device, seed) -> None:
    """Note(Mandi): changed to use _envs_ object"""
...
def main(cfg: DictConfig) -> None:
    ...
    log_path = join(cwd, tasks_name, cfg.method.name)
    ...
    for seed in range(existing_seeds, existing_seeds + cfg.framework.seeds):
        ...
        run_seed(cfg, envs, cfg.rlbench.cameras, device, seed)

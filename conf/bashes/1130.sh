...
HID=1024
RUN=10Var-dVAE-QEncode${DIM}-1Demo-1Layer-Hidden${HID}-OneHot
taskset -c $CPUS  python launch_context.py tasks=['pick_up_cup'] rlbench.num_vars=10 run_name=$RUN \
dataset.num_steps_per_episode=1 dataset.defer_transforms=True dev.discrete=True contexts.loss_mode='dvae' \
dev.qnet_context_latent_size=${DIM} framework.training_iterations=20000 \
contexts.sampler.k_dim=1  dev.single_layer_context=True dev.encode_context_hidden=${HID} contexts.discrete_agent.one_hot=True
...
DIM=16
HID=1024
RUN=20Var-dVAE-QEncode${DIM}-1Demo-1Layer-Hidden${HID}-OneHot
taskset -c $CPUS  python launch_context.py tasks=['pick_up_cup'] rlbench.num_vars=20 run_name=$RUN \
dataset.num_steps_per_episode=1 dataset.defer_transforms=True dev.discrete=True contexts.loss_mode='dvae' \
dev.qnet_context_latent_size=${DIM} framework.training_iterations=20000 \
contexts.sampler.k_dim=1  dev.single_layer_context=True dev.encode_context_hidden=${HID} contexts.discrete_agent.one_hot=True
...
DIM=16
HID=1024
RUN=10Tasks-dVAE-QEncode${DIM}-1Demo-1Layer-Hidden${HID}-OneHot
taskset -c $CPUS  python launch_context.py tasks='${10_tasks}' rlbench.num_vars=1 run_name=$RUN \
dataset.num_steps_per_episode=1 dataset.defer_transforms=True dev.discrete=True contexts.loss_mode='dvae' \
dev.qnet_context_latent_size=${DIM} framework.training_iterations=50000 \
contexts.sampler.k_dim=1  dev.single_layer_context=True dev.encode_context_hidden=${HID} \
contexts.discrete_agent.one_hot=True replay.buffers_per_batch=10 replay.batch_size=6
